...
\item law of cheap gradient in adjoint mode (complexity = constant low multiple\footnote{theory claims that the multiple is bounded by 4} of one function evaluation)
\item implementation by hand coded derivative code, operator overloading tools instrumenting code at runtime or source code transformation generating derivative code at compile time
\end{itemize}
\end{frame}
...
\item QuantLibAdjoint project (CompatibL / Alex Sokol), with additional logic (Tapescript project)
...
\item Penalty: 11.7x\footnote{\tiny different OO tools my show different penalties; a MinimalWrapper consisting of a double and a native MinimalWrapper - pointer (which is null always), the penalty is around 2.0x}
...
\item the additional computation time is quite small (see the example above, the addtional costs are similar to 1.5x original NPV calculations)
\end{itemize}
\end{frame}
...
\item source code transformation can solve this issue, we gave an example in terms of a bermudan swaption engine transformed using OpenAD/F yielding an adjoint multiple of 6.1 compared to 80 with CppAD
\end{itemize}
\end{frame}
